...
// [2] don't use std::aligned_storage
// [3] g_vt instead of local static vt
// [4] don't call destructor if is_trivially_destructible
...
// [2]
template <size_t Cap, size_t Aligment>
struct inplace_aligned_storage {
    alignas(Aligment) std::byte data[Cap];
};

template<size_t Cap, size_t Align = alignof(aligned_storage_helper<Cap>)>
struct aligned_storage {
    using type = inplace_aligned_storage<Cap, Align>;
};
...
template<class R, class... Args> struct vtable
{
    ...
    explicit constexpr vtable() noexcept :
        invoke_ptr{ [](storage_ptr_t, Args&&...) -> R
            { ION_UNREACHABLE("bad function call"); } // [1]
        },
...
// [3]
template<typename vtable_t, typename C>
static const vtable_t g_vt{ inplace_function_detail::wrapper<C>{} };
...
public:
    ...
    inplace_function(T&& closure)
    {
        ...
        vtable_ptr_ = std::addressof(g_vt<vtable_t, C>);
    ...
    ~inplace_function()
    {
        if (!std::is_trivially_destructible<R>()) // <-- avoid indirect fn call for simple cases
        {
            vtable_ptr_->destructor_ptr(std::addressof(storage_));
        }
    }
